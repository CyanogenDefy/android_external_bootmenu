...
static int vk_init(struct ev *e)
{
    ...
    len = strlen(vk_path);
    len = ioctl(e->fd->fd, EVIOCGNAME(sizeof(vk_path) - len - 1), vk_path + len);
    if (len <= 0)
        return -1;
    ...
    */
    ...
    e->vks = malloc(sizeof(struct virtualkey) * e->vk_count);
    memset(e->vks, 0, sizeof(struct virtualkey) * e->vk_count);
